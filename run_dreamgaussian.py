import os
os.environ["PYTHONIOENCODING"] = "utf-8"  # Ensure UTF-8 output for Windows console
...
class GUI:
    def __init__(self, opt):
        ...
        # Ensure save_path is set, fallback to opt.name or 'dreamasset' if missing
        try:
            _ = self.opt.save_path
        except Exception:
            self.opt.save_path = getattr(self.opt, "name", "dreamasset")
    ...
    def train_step(self):
        ...
        for _ in range(self.train_steps):
            ...
            loss = torch.tensor(0.0, device=self.device, requires_grad=True)  # ensure loss requires grad
    ...
    def save_model(self, mode='geo', texture_size=1024):
        # Ensure save_path is set before saving
        if not hasattr(self.opt, "save_path") or self.opt.save_path is None:
            self.opt.save_path = getattr(self.opt, "name", "dreamasset")
    ...
    def train(self, iters=500):
        # Ensure save_path is set before training
        if not hasattr(self.opt, "save_path") or self.opt.save_path is None:
            self.opt.save_path = getattr(self.opt, "name", "dreamasset")
        if iters > 0:
            self.prepare_train()
